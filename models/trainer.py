...
class Trainer():
    ...
    def _train_iteration(self, data: list, label: torch.tensor) -> float:
        ...
        return loss.item()*batch_size, batch_corrects
    ...
    def _train_epoch(self, trainloader: torch.utils.data.DataLoader, testloader: torch.utils.data.DataLoader) -> None:
        """ """
    ...
    def train(self,
        ...
        for epoch in range(1, epochs+1):
            ...
            if epoch % self.print_every == 0:
                ...
                print("Epoch {} {:.2f}s: train loss {:.6f}, train acc {:.3f}, test loss {:.6f}, test acc {:.3f}".format(
                    epoch, time.time()-start, self.losses['train'][-1], self.acces['train'][-1], self.losses['test'][-1], self.acces['test'][-1]
                ))
